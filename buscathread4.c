...
void * lerarquivo1(){
	...
	if (( arquivo = fopen("File1_test3.txt", "r")) == NULL ) {
...
void * lerarquivo2(){
	...
	if (( arquivo = fopen("File2_test3.txt", "r")) == NULL ) {
        printf("Falha ao abrir o arquivo");
        exit(1);
    }
	//inicia contagem do tempo de exec
...
void * lerarquivo3(){
	...
	if (( arquivo = fopen("File3_test3.txt", "r")) == NULL ) {
        printf("Falha ao abrir o arquivo");
        exit(1);
    }
	//inicia contagem do tempo de exec
	...
	while(fgets(palavra, 128, arquivo) != NULL) {
		if((strstr(palavra, string)) != NULL) {
			printf("\nString encontrada no arquivo 3 pela thread 3\n");
			strFlag++;
			//termina contagem do tempo de exec
			tempoFin = clock();
			//converte o tempo de exec em ms
...
void * lerarquivo4(){
	...
	if (( arquivo = fopen("File4_test3.txt", "r")) == NULL ) {
        printf("Falha ao abrir o arquivo");
        exit(1);
    } 
	...
	while(fgets(palavra, 128, arquivo) != NULL) {
		if((strstr(palavra, string)) != NULL) {
			printf("\nString encontrada no arquivo 4 pela thread 4\n");
			strFlag++;
			//termina contagem do tempo de exec
			tempoFin = clock();
			//converte o tempo de exec em ms
...
pthread_create(&t1, NULL, lerarquivo3,NULL);
pthread_create(&t1, NULL, lerarquivo4,NULL);
